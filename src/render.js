...
import { UI, UI_FONT, drawHUD, drawTitle, drawSelect, drawIntroCard, drawAidCard, drawDNFCard, drawFinishCard, drawToasts, drawPause } from './ui.js';
...
export function render(ctx, tReal) {
  ...
  ctx.restore();
  const menu = GAME.screen === 'title' || GAME.screen === 'select';
  if (!menu) drawHUD(ctx);
  UI.buttons = [];
  if (GAME.screen === 'title') drawTitle(ctx);
  else if (GAME.screen === 'select') drawSelect(ctx);
  else if (GAME.screen === 'intro') drawIntroCard(ctx);
