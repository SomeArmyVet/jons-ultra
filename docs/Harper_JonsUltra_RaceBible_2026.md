...
- Animals (corrected 2026-09-05; telegraphs added 6e — every biter telegraphs): wild pig (waits trailside, 0.6 s telegraph — grunt, hoof dust, head lowers — then bolts across in 0.5 s; jump clears it, duck does not; roams all day, most active dawn/dusk per the Book; bite = hit), mongoose (ferret-shaped, darts underfoot — small stumble, no hit), feral chicken (harmless flavour, scatters in small groups), brush-tailed rock wallaby (coils visibly for 0.45 s, then hops across in one high arc at dawn/dusk/night — collide = hit, duck under the arc; REAL: the Kalihi Valley colony, Petrogale penicillata, descended from a pair that escaped Richard Trent's private zoo on 20 Aug 1916 — sources: UH Mānoa ScholarSpace mtDNA study; Hawaii DLNR 2020-06-25 news release; Kalihi is the next valley from the Nu'uanu leg), giant centipede (on the trail at night, bite = hit), rat at night (harmless). Animal eyes catch the headlamp beyond the cone at night (eye-shine — real, and it keeps night encounters fair). Biter encounters are placed by a phase-aware per-loop quota (`hitQuotaPerLoop`, Architecture §4) so every loop carries 3–8 hits' worth of pressure.
- Lore hazard — Huakaʻi Pō (Night Marchers), added 2026-09-05, treated respectfully; guaranteed once per race since 6d/6e: torches walk the far ridge with low synthesised drums on every night — no chants, no faces, no caricature — and exactly one procession crosses the trail per race, during night, at a seeded mile. The legend's answer — do not look, get low — maps to the game's DUCK (or crouch), held until they pass; Jon stops while he holds it. Standing or jumping = a hit and the screen darkens for a beat. [Michael confirmed: DUCK; jumping or standing = hit.]
